...
const Roadmap = ({ pageId }) => {
  ...
  const [student, setStudentEmail] = useState('');
  const [correlators, setCorrelators] = useState([]);
  const [relator, setRelatorEmail] = useState('');
  ...
  useEffect(() => {
    const fetchRoadmapData = async (pageId) => {
      try {
        ...
        setStudentEmail(emailToName(data.studenteEmail));
        setRelatorEmail(emailToName(data.relatoreEmail));
  ...
  return (
    <div className={styles.appContainer}>
        <>
          ...
          <div className={styles.backButton} onClick={() => router.push('/collaboratore/visualizzaPagineUnitesiCollaboratore')}>
            <VisualizzaPagineUnitesi />
          </div>

          <div>
            <div className={styles.membriContainer}>
                <h2>Studente: {student}</h2>
                <h3>Relatore: {relator} </h3>
